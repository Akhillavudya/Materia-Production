...
export const TOOL_FORMS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'atom_index', label: 'On atom index (blank = use site)', type: 'number', default: '', step: 1, min: 0, placeholder: 'e.g. 5' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'custom_kppa', label: 'Custom kppa', type: 'number', default: '', min: 1,
        showIf: (v) => v.accuracy_level === 'Custom' },
      { name: 'explicit_mesh', label: 'Exact mesh — overrides accuracy (e.g. 2 2 2)', type: 'text',
        default: '', placeholder: '2 2 2 or 1 1 1' },
  ...
  // ── instant defect tools: build a defective supercell as the active POSCAR ──
  {
    ...
    color: '#C9536B',
    icon: 'vacancy',
    description: 'Remove one, several, or all atoms of an element to make vacancies.',
    endpoint: 'create_vacancy',
    instant: true,
    fields: [
      { name: 'element', label: 'Element (blank = first)', type: 'text', default: '', placeholder: 'Si' },
      { name: 'count', label: 'How many (number or "all")', type: 'text', default: '1', placeholder: '1, 3, or all' },
    ],
  },
  {
    ...
    color: '#CB6553',
    icon: 'substitution',
    description: 'Replace one, several, or all atoms of one element with another.',
    endpoint: 'create_substitution',
    instant: true,
    fields: [
      { name: 'from_element', label: 'From', type: 'text', default: '', placeholder: 'Si', required: true },
      { name: 'to_element', label: 'To', type: 'text', default: '', placeholder: 'Mg', required: true },
      { name: 'count', label: 'How many (number or "all")', type: 'text', default: '1', placeholder: '1, 3, or all' },
    ],
  },
  {
    ...
    color: '#C77A45',
    icon: 'interstitial',
    description: 'Insert one, several, or all atoms at Voronoi interstitial sites.',
  ...
  },
  {
    ...
    color: '#D4537E',
    icon: 'sqs',
    description: 'Random alloy (SQS) from a normal structure — pick a site, set its composition.',
    endpoint: 'sqs',
    // The SQS search is correlation-based (no potential), but the result is
    // relaxed with an ML potential — so the card shows a Potential picker.
    calculator: true,
    analyze: 'sublattices',   // enables the "Detect sublattices" pre-flight button
    heavy: true,
    fields: [
      { name: 'sublattice_comp', label: 'Sublattice composition', type: 'text', default: '',
        full: true, placeholder: 'Ti=Ti0.6,Zr0.4 ; Sr=Sr0.6,Ba0.4' },
      { name: 'supercell', label: 'Supercell', type: 'text', default: '2 2 2' },
      { name: 'relax', label: 'Relax result (MLP)', type: 'checkbox', default: true },
  ...
  },
  {
    ...
    endpoint: 'neb',
    calculator: true,
    twoFiles: true,         // file pickers (shown only in "Two files" mode)
    analyze: 'migration',   // "List hops" + clickable chips (shown in element mode)
    heavy: true,
    fields: [
      { name: 'mode', label: 'Build endpoints from', type: 'select', default: 'element', full: true,
        options: [['element', 'A migrating element (auto-build)'],
                  ['files', 'Two files (initial + final)']] },
      { name: 'migrating_element', label: 'Migrating element', type: 'text', default: '', placeholder: 'Mg',
        showIf: (v) => v.mode === 'element' },
      { name: 'cutoff', label: 'Max hop (Å)', type: 'number', default: 5.0, step: 0.5, min: 1,
        showIf: (v) => v.mode === 'element' },
      { name: 'source_site', label: 'Source site (blank = shortest)', type: 'number', default: '', min: 1,
        showIf: (v) => v.mode === 'element' },
      { name: 'dest_site', label: 'Dest site', type: 'number', default: '', min: 1,
        showIf: (v) => v.mode === 'element' },
      { name: 'n_images', label: 'Images', type: 'number', default: 8, min: 3, max: 15 },
      { name: 'endpoint_fmax', label: 'Endpoint fmax (eV/Å)', type: 'number', default: 0.03, step: 0.01, min: 0.001 },
      { name: 'climb', label: 'Climbing image', type: 'checkbox', default: true },
      { name: 'run_frequencies', label: 'Confirm saddle (Hessian)', type: 'checkbox', default: true },
      { name: 'emit_vasp_inputs', label: 'Emit VASP decks (zip)', type: 'checkbox', default: true },
    ],
  },
]
